...
-- 8) emp테이블에서 empno, ename, job, hiredate(입사일)
--    을 조회하시오
SELECT empno
      , ename
      , job
...
*/
-- ORDER BY: 정렬
-- 9) emp 테이블에서 comm을 가장 많이 받는 순서대로(=역순:내림차순 DESC)
--    empno, ename, job,hiredate, comm칼럼을 조회하시오
SELECT empno
      ,ename
      ,job
      ,hiredate
...
7844	TURNER	SALESMAN	81/09/08	0
  */
-- 10) emp 테이블에서 
--    comm은 적은 순서대로, job은 오름차순(영문), ename오름차순(영문)
--    정렬하여
...
7839	KING	PRESIDENT	81/11/17	
  */
-- emp 테이블에서
-- comm이 적은 순서대로, job은 오름차순, ename은 내림차순으로 정렬하여
-- empno, ename, job, hiredate, comm을 조회하시오
SELECT empno
      ,ename
      ,job
...
SELECT empno AS"Employee NO."
     ,ename "Employee name" --AS 키워드는 생략 가능
...
7934	MILLER
*/
--14) 영문 별칭을 사용 할 때, 쌍따옴표를 사용하지 않으면
...
;
/*
EMPLOYEENO, EmployeeName,   Job Name
-----------------------------------------
7369	    SMITH	        CLERK
7499	    ALLEN	        SALESMAN
7521	    WARD	        SALESMAN
...
--15) 별칭과 정렬의 조합: 
--    컬럼에 별칭을 준 경우 ORDER BY 절에 별칭을 사용할 수 있다.
...
SELECT e.empno 사번
    ...
    ,e.comm 수당
  FROM emp e
ORDER BY 수당, 직무, e.ename --정렬을 할 때 별칭을 쓸 수 있고, 컬럼 명을 그대로 써도 됨
;
--(10)번과 동일하나 별칭을 사용한 부분만 차이가 남.
--    =>결과: ODER BY 절에는 별칭과 원래 컬럼명을 섞어 쓸 수 있다.
/*
사번,     이름,  직무,        입사일,      수당
-------------------------------------------------
7844	TURNER	SALESMAN	81/09/08	0
