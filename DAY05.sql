...
SELECT e.empno
 ...
 WHERE e.job='CLERK'
   ...
   AND e.mgr=7698
;
-- AND연산자는 OR 연산자 보다 우선순위가 높기 때문에
...
SELECT e.empno
  ...
  FROM emp e
;
-- 단일행 함수는 테이블의 1행당 한 번씩 적용된다.

------- 2. ROUND(m, n) : 실수 m을 소숫점 n + 1 자리에서 반올림 한 결과 계산
...
-------------------------3. TRUNC(m,n) : 실수 m을 n에서 지정한 자리 아래 소숫점 버림
SELECT TRUNC(1234.56,0) --1234
  FROM dual
;
------------ TRUNC(m) : n이 생략된 경우에는 n=0으로 수행한다.
--                      즉 , 소숫점 이하 모두 버림
...
-----------------------5. FLOOR(n) : 입력된 실수 n에서 같거나 가장 가까은 작은 정수
