...
SELECT 'Hello, SQL~' AS greeting--고정된 값을 조회하면 어떻게 될까?
       ,empno -- 테이블에 존재하는 컬럼명을 섰어서 쓰면 어떻게 될까?
       ,ename
  FROM emp
;
/*
...
--18) 17번의 조회 조건(empno가7900, deptno가 20)을 AND로 결합하여
--    조회해보자.
...
--19) job이 'CLERK이면서 deptno가 10인 직원의
...
SELECT e.empno
       ...
       ,e.deptno
  FROM emp e
 WHERE e.job='CLERK'-- 문자열을 비교할 때에는 반드시 비교 문자를 ''로 감싼다. 
