...
/* DDL 구문의 시작
   ...
   SCHEMA, DOMAIN, TABLE, VIEW, INDEX, SWQUENCE, USER, DATABASE 
...
/* -----------------------------------
   컬럼의 제약사항
   -----------------------------------
   1. PRIMARY KEY : 이 컬럼의 입력되는 값은 중복되지 않고
                    한 행을 식별할 수 있는 값으로 설정해야 하며
                    null 데이터 입력은 불가능한 값이어야 한다.
                    
   2. FOREING KEY : 주로 JOIN에 사용되는 제약조건으로
                    다른 테이블의 PRIMARY키로 사용되었던 값이
                    등장해야만 한다.
...
-- , CONSTRAINT 제약조건이름 제약조건 타입 (제약조건 적용대상 컬럼)
 ...
 , CONSTRAINT pk_member_gender  CHECK        (gender IN ('F','M'))
);
...
-- 현재 유저가 가지고 있는 제약조건(constraints) 들의 정보가 저장되는 
-- 시스템 카탈로그 테이블
...
PK_DEPT	                        P	DEPT
PK_EMP	                        P	EMP
PK_MEMBER_GENDER	            C	MEMBER
PK_MEMBER	                    P	MEMBER
BIN$xAz8BrelSlWvR3pWnKMtkg==$0	C	BIN$PvIPtHSkQ46IfPzjwzDmfg==$0
*/
...
-- 예) 앞서 생성한 member테이블에서 복사 : new_member
...
-- 1) 컬럼에 데이터가 없을 떄
--    : 모든 변경에 자유로움
--      데이터 타입 변경, 데이터 크기변경에 모두 자유로움

-- 2) 컬럼에 데이터가 있을 때
--    : 데이터가 소실되면 안되므로 변경에 제약이 있음
--      타입 변경은 같은 타입 내에서만 가능
...
-- BIRTH_MONTH 컬럼의 값을 NULL 데이터로 변경
...
(   ID        VARCHAR2(10)   CONSTRAINT PK_MAIN PRIMARY KEY
  ...
  , REG_DATE  DATE           DEFAULT SYSDATE
  , GENDER    VARCHAR2(1)    CONSTRAINT CK_GENDER CHECK(GENDER IN ('F','M'))
  , MESSAGE   VARCHAR2(300)
);
...
(   ID        VARCHAR2(10)   
  ...
  , CONSTRAINT CK_GENDER3   CHECK (GENDER IN ('F','M'))
);
...
(   ID         VARCHAR2(10)  
   ...
   -- SUB_TABLE3의 경우 PRIMARY_KEY 를 ID, BITH_YEAR 의 복합키로 생성
   -- 복합기로 PK를 삼으려는 경우는 반드시 제약조건 추가로만 생성가능
...
(   CONSTRAINT PK_MAIN4     PRIMARY KEY (ID)
  ...
  , CONSTRAINT CK_GENDER4   CHECK (GENDER IN ('F','M'))
);
...
--------------------------------------------
MAIN_TABLE1	SYS_C007520	        P(= primary key)
MAIN_TABLE1	SYS_C007519	        C(= check)
MAIN_TABLE1	SYS_C007521	        U(= unique)
...
SUB_TABLE1	SYS_C007522	        R(= reference ==> foreign key)
SUB_TABLE2	FK_SUB	            R
...
-- 테이블 삭제 : DROP
-- 두 테이블 사이에 REFERENCE (FOREIGN KEY) 관계가 있을 떄 삭제

-- MAIN_TABLE1의 경우 SUB_TABLE1에 의해 ID 컬럼이 참조되고 있는 상태
...
*/
-- SUB_TABLE1이 MAIN_TABLE1의 ID컬럼을 참조하고 있기 때문에 
-- 테이블 삭제 시 순서가 필요함.
...
-- 위의 쿼리 결과 : 인출된 모들 행 0이 된다.
-- 즉, 제약조건을 모두 삭제하며 테이블을 DROP함.
-- 특히 이 결과에서 SUB_TABLE2에 있던 R제약조건이 같이 사라졌음을 확인

-- SUB_TABLE3 을 DROP한 뒤, USER CONSTRAINTS 에서 관련 행이 사라짐을 확인
DROP TABLE SUB_TABLE3;
...
MAIN_TABLE3	    PK_MAIN3	    P 
*/
-- SUB_TABLE3 을 DROP 해도 MAIN_TABLE3의 제약조건에는
-- 영향을 미치지 않음을 확인.
