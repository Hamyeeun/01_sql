...
SELECT upper('sql is cooooooooooooool~!!!') "재밌어요!"
  FROM dual
;
...
--               str의 글자 길이, 글자의 byte 를 계산하여 숫자로 출력
SELECT length('hello, sql') as "글자 길이"
  FROM dual
;
-- 결과: 10 ==> 특수기호, 공백도 글자 길이로 계산
SELECT 'hello, sql의 글자길이는'||length('hello, sql')
                                 || '입니다.' "글자 길이"
  FROM dual
;
/*
...
------------- 5. COMCAT(str1, str2) : str1과 str2를 문자열 접합
--                                   || 연산자와 동일한 결과
...
---------------- 6. SUBSTR(str, i, n) : 
--                  str 에서 i 번째 위치에부터 n 개의 글자를 추출
--                  SQL에서 문자열의 인덱스(순서)는 1부터 시작
SELECT substr('SQL is coooooooooo1~~!!',3,4)
  FROM dual
;
/*
SUBSTR('SQLISCOOOOOOOOOO1~~!!',3,4)
-------------------------------------
L is
*/
--------- SUBSTR(str, i) : 
--        str 에서 i 번째 위치에서 끝까지 글자를 추출
SELECT substr('SQL is coooooooooo1~~!!',3)
  FROM dual
;
/*
SUBSTR('SQLISCOOOOOOOOOO1~~!!',3,4)
...
-- 문제: SQL is coooooooooo1~~!! 에서 SQL 만 추출해보자

SELECT substr('SQL is coooooooooo1~~!!',1,3)
  FROM dual
;

-- 문제: SQL is coooooooooo1~~!! 에서 is 만 추출해보자

SELECT substr('SQL is coooooooooo1~~!!',5,2)
  FROM dual
;

-- 문제: SQL is coooooooooo1~~!! 에서 ~~!! 만 추출해보자

SELECT substr('SQL is coooooooooo1~~!!',20,4)
  FROM dual
;
...
SELECT instr('SQL is coooooooooo1~~!!','is') "is의 위치"
  FROM dual
;
/*
is의 위치
...
SELECT instr('SQL is coooooooooo1~~!!','io') "io의 위치"
  FROM dual
;
/* 두 번째 문자열이 첫번째 문자열에 없으면 0을 출력한다.
io의 위치
----------
        0
*/
...
--  NULL값을 대체하는 '매니저 없음'은 문자타입의 데이터 이므로
...
SELECT e.empno
      , e.ename
      , nvl2(concat(e.mgr,''),'매니저 있음','매니저 없음') "매니저 유/ 무" --expr2 과 expr3의 데이터 타입은 맞춰야함
  FROM emp e
;
/*
EMPNO,  ENAME,      매니저 유/ 무
...
-- NULLIF(expr1, expr2) : 
-- 첫 번째 식, 두번째 식의 값이 동일하면 NULL을 출력
-- 두값이 다르면 첫번째 식의 값을 출력
...
---------- 3) 날짜 함수 : 날짜 관련된 출력 / 날짜의 더하기 빼기 등의 연산을
--                        할 수 있는 기능을 제공
...
SELECT to_char(sysdate,'MM') "달" --05
  FROM dual
;
SELECT to_char(sysdate,'MONTH') "달" --5월
  FROM dual
;

SELECT to_char(sysdate,'MON') "달" --5월
  FROM dual
;
...
SELECT to_char(sysdate,'FMYYYY-MM-DD') "오늘날짜" --2019-5-24
  FROM dual
;
SELECT to_char(sysdate,'YY-MON-DD') "오늘날짜" --2019-5월-24
  FROM dual
;
SELECT to_char(sysdate,'YYYY-MM-DD DAY') "오늘날짜" --2019-05-24 금요일
  FROM dual
;

SELECT to_char(sysdate,'YYYY-MM-DD DY') "오늘날짜" --2019-05-24 금
  FROM dual
;
/*
시간패턴 : 
...
SELECT to_char(sysdate,'YYYY-MM-DD HH24:MI:SS') "오늘날짜 시분초" --2019-05-24 15:30:05
  FROM dual
;


SELECT to_char(sysdate,'YYYY-MM-DD HH:MI:SS') "오늘날짜 시분초" --2019-05-24 03:41:54
  FROM dual
;

SELECT to_char(sysdate,'YYYY-MM-DD AM HH:MI:SS') "오늘날짜 시분초" --2019-05-24 오후 03:41:54 
...
SELECT e.empno
      , e.ename
      , round(MONTHS_BETWEEN(sysdate, e.hiredate),1) "근속 개월수"
  FROM emp e
;
/*
EMPNO,  ENAME,      근속 개월수
...
SELECT round(sysdate) "현재시간에서 반올림"
  FROM dual
;

-- 현재 시간에서 반올림된 날짜를 시/분/초 까지 출력
SELECT to_char(round(sysdate),'YYYY-MM-DD HH24:MI:SS') "현재시간에서 반올림 시/분/초"
  FROM dual
; 
--2019-05-25 00:00:00
